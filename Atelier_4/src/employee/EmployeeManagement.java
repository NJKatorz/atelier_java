...
public class EmployeeManagement {
    ...
    private static final Supplier<Stream<String>> supplier = () -> {
        ...
        try {
            return Files.lines(Path.of("./Atelier_4/resources/streamingvf.csv"));
        } catch (IOException e) {
    ...
    private static String firstLine() {
        try (Stream<String> line = supplier.get()) {
            return line.findFirst().get();
    ...
    private static List<String> filteredLastnames() {
        ...
        predicate = predicate.and(e -> e.toString().contains("O") || e.toString().contains("K"));
        Stream<String> line = supplier.get();
        return line
                .map(Employee::new)
    ...
    private static List<Integer> occurencesOfE() {
        ...
        BiFunction<Employee, Character, Integer> result = (e, c) -> (int) e.getFirstname().chars().filter(ch -> ch == c).count();

        return supplier.get()
                .map(Employee::new)
                .map(e -> result.apply(e, 'e'))
                .collect(toList());
    }
    ...
    private static Map<Boolean, List<Integer>> timeDistrubution() {
        Map<Boolean, List<Integer>> map = supplier
                .get()
                .map(Employee::new)
                .collect(partitioningBy(Employee::isFullTime, mapping(Employee::getId, toList())));
        return map;
    }
    ...
    private static void printLongestName() {
        withLines(lines -> {
            ...
            System.out.println(lines.map(Employee::new)
                    .map(Employee::getLastname)
